...
<div class="ldg_box l_big">
		  <?= $this->Form->input('name', array(
            'label'=>'Ledger name', 'empty'=> 'choose one', 'options'=> $LDG_name, 'type'=>'select', 'style'=>'border:1px solid #ccc; background:url(../img/bg.png) repeat-x bottom;background-position:center'
        )); ?>

<?=$this->Form->input('date_from', ['id'=> 'birthday', 'type'=>'text', 'class'=>'dd', 'dateFormat'=> 'YMD', 'minYear'=> date('Y') - 80, 'maxYear'=> date('Y') - 18, 'label'=> 'Date from', ]);
						?>

<?=$this->Form->input('date_to', ['id'=> 'employ_date', 'type'=>'text', 'class'=>'dd', 'dateFormat'=> 'YMD', 'minYear'=> date('Y') - 80, 'maxYear'=> date('Y') - 18, 'label'=> 'Date to', ]);
                    ?>

		<?= $this->Form->input('amount', array(
            'label'=>'Amount', 'type'=>'text', 'style'=>'width:110px'
        )); ?>
