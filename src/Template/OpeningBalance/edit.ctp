...
<hr />
	<?php
		echo $this->Form->create($OpeningBalance);
			...
			echo $this->Form->input('LDG_ID', ['label'=> 'Ledger Name', 'options'=> $lgd_name1, //'selected'=> 'selected', 'type'=>'select', 'style'=>'border:1px solid #ccc; background:url(../img/bg.png) repeat-x bottom;background-position:center'
			]);

echo $this->Form->input('balance_date', array(
                    'label'=> 'Balance Date', 'id'=> 'balance_date', 'type'=>'text', 'style'=> 'color:#f26522; font-weight:600;', ));

            echo $this->Form->input('LBL_BALANCE_DR', array(
                    'label'=> 'Balance DR', 'type'=>'text'
             ));

            echo $this->Form->input('LBL_BALANCE_CR', array(
                    'label'=> 'Balance CR', 'type'=>'text'
             ));
