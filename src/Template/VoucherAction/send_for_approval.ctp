...
<?php
if ($noform==false){?>
			<h3 style="border-bottom:1px solid #060;padding:6px;background:#060;color:#FFF;">Send For Approval</h3>
			<?= $this->Form->create() ?>

		  <?= $this->Form->input('name', array(
            'label'=>'Send To', 'options'=> $user_name, 'type'=>'select', 'style'=>'border:1px solid #ccc; background:url(../img/bg.png) repeat-x bottom;background-position:center;width:200px;'
        )); ?>

<?= $this->Form->input('DESC', array(
            'label'=>'Description', 'type'=>'textarea', 'style'=>'border:1px solid #ccc; background:url(../img/bg.png) repeat-x bottom;background-position:center;width:200px; height:80px;'
        )); ?>

<?=$this->Form->button('Send', array('class'=>'custom_submit', 'style'=>'float:left;margin-bottom:10px;'));  ?><br />
  <?= $this->Form->end() ?>
