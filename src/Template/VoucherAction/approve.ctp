...
<?= $this->Form->input('VCH_ID', array(
            'label'=>'Voucher ID', 'value'=>$vch_id, 'type'=>'hidden', )); ?>

        	  <?= $this->Form->input('DESC', array(
            'label'=>'Notes', 'style'=>'width:90%', 'type'=>'textarea', )); ?>
<br /><br /><br /><br />
<div style="border-bottom:1px solid #ddd;margin-top:12px;width:100%">&nbsp;</div>

<?= $this->Form->button('APPROVE', array(
			'value'=>STS_APPROVED, 'name'=>'APPROVE', 'label'=>'', 'style'=>'font-size:14px;float:left;width:100px;margin:3px;margin-bottom:10px;'
        )); ?>

<?= $this->Form->button('DENIED', array(
			'value'=>STS_DENIED, 'name'=>'DENIED', 'label'=>'', 'style'=>'font-size:14px;float:left;width:100px;margin:3px'
