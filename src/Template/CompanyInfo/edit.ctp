...
echo $this->Form->input('CMP_ROOT_ID', array(
					'label'=>'Group Name', 'type'=>'select', 'options'=>$CompanyRoot
				)
			);

			/*echo $this->Form->input('CMP_ROOT_ID', array(
				'options'=> $CompanyInfo, 'label'=> 'Company Root Id', 'type'=> 'select', 'selected'=>'selected'
			));*/
			...
			echo $this->Form->input('CMP_ACTIVE', array(
				'label'=> 'Active Status', 'type'=> 'checkbox', 'class'=> 'check'
			));
			/*
