	  ...
	  public function add()
   		 {
...

				{
					...
					if ($this->request->is('post'))
					{
						...
						$chk=($this->request->getData()["LDG_chk"]);

						$c_balance=($this->request->getData()["LDG_CURRENT_BALANCE"]);

						$balance_date=($this->request->getData()["balance_date"]);
				...
				$Acc_type=($this->request->getData()["LDG_ACC_TYPE"]);
						...
						$lid='';
							...
							$rdo=($this->request->getData()["LDG_rso"]);
							...
							$invoice=$this->request->getData('balance_date');
