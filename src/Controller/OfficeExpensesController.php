	...
	$query=$this->OfficeExpenses->Ledgerstype->find('list',['valueField' => 'LDG_ID'])
        ...
        if ($this->request->is('post'))

		 {
			...
			$from=ACC_CASH;
			$ldg_name=($this->request->getData()["VCH_DR_ACCOUNTS"]);

            $OfficeExpenses = $this->OfficeExpenses->patchEntity($OfficeExpenses, $this->request->data);

			$OfficeExpenses->VCH_AMOUNT=$this->request->getData()["VCH_AMOUNT"];
			...
			$invoice=$this->request->getData('from_date');
...
public function edit($VCH_ID = null)
{
...

	{
			...
			$amount=($this->request->getData()["VCH_AMOUNT"]);
			$from=($this->request->getData()["VCH_CR_ACCOUNTS"]);
			$ldg_name=($this->request->getData()["VCH_DR_ACCOUNTS"]);
