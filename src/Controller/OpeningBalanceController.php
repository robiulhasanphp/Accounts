...
namespace App\Controller;
	...
	class  OpeningBalanceController extends AppController{
		...
		if ($this->request->is(['post', 'put'])) {
			$search_period=$this->request->getData()['LDG_BAL_PERIOD'];
		}
	 ...
	 public function add()
		...
		$this->set(compact('LDG_name'));
			...
			if ($this->request->is('post')) {
				...
				$b_date = $this->request->getData('balance_date');
				...
				$ldg_id = $this->request->getData()['LDG_ID'];
	...
	//		->where(['LDG_ID IN ' =>$pur]);
			...
			if ($this->request->is(['post', 'put'])) {
				...
				$OpeningBalance->LDG_BAL_DATE=date('Y-m-d',strtotime($this->request->getData()['balance_date']));
				...
				$lgd_bal_id = $this->request->getData('LDG_BAL_ID');
