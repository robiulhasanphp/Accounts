   ...
   //echo "<pre>";
  	...
  	public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			$VoucherAction_from=($this->request->getData()["VCH_CR_ACCOUNTS"]);
...
public function SendForApproval($VCH_ID)
	...
	$user_id=$user["USR_ID"];
				...
				$Basicdata=$Users=$this->fetchTable(Basicdata;
...
/*check*/
		 ...
		 $vch_id=$VCH_ID;
...

			{
				...
				$TEXT=($this->request->getData()["DESC"]);
				$ACC_TYPE=STS_SENT;
...
$DENIED=0;
		  $TEXT=($this->request->getData()["DESC"]);
		  if(isset($this->request->getData()["APPROVE"]))
		  $ACC_TYPE=($this->request->getData()["APPROVE"]);

		  if(isset($this->request->getData()["DENIED"]))
  		  $DENIED=($this->request->getData()["DENIED"]);
